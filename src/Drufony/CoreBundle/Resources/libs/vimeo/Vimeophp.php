...
namespace Drufony\CoreBundle\Resources\libs\vimeo;
...
*/
    public function call($method, $params = array(), $request_method = 'GET', $url = self::API_REST_URL, $cache = true)
    {
        ...
        try {
            $request = $this->_request($method, $params, $request_method, $url, $cache);
...
*/
    ...
    {
        if (is_array($input)) {
            return array_map(array(__CLASS__, 'url_encode_rfc3986'), $input);
        }
     ...
     * @return true. The only possible error is not found video
     */
     public function delete($videoID)
     {
        $params = array('video_id' => $videoID);
        try {
            $error = $this->call('vimeo.videos.delete', $params, 'GET', self::API_REST_URL, false);
        }catch(Exception $e) {
             throw $e;
        }
        return true;
    ...
    /**
     * Sets the title for a given video
     * @param $videoID The id of the video
     * @param $videoTitle The title we're setting
     */
    public function setTitle($videoID, $videoTitle)
    {
        $params = array('video_id' => $videoID, 'title' => $videoTitle);
        try{
            $error = $this->call('vimeo.videos.setTitle', $params, 'GET', self::API_REST_URL, false);
        }catch(Exception $e) {
            throw $e;
        }
        return true;
